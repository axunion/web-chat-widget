...
interface ChatWidgetOptions {
  ...
  store?: ChatStore;
  maxInputLength?: number;               // maxlength applied to the input textarea
}
...
| `stop` | `(): void` | Aborts the in-flight response. Partial assistant content is kept and settled to `status: "done"`; an empty streaming placeholder is removed (the user message stays). Does **not** fire the `message` event. No-op when idle. |

### 2.2.1 Properties

| Property | Type | Description |
| --- | --- | --- |
| `busy` | `boolean` (read-only) | `true` while a response is in flight — from send until the exchange settles (`done` / `error` / `stop()`). |
...
| `message` | `{ role: "user" \| "assistant"; content: string }` | An assistant response is confirmed on `done` chunk (once per message), or a user message is appended by a send (`retry()` does not re-fire it for the resent user message). `"system"` role is excluded. Stopped (partial) responses do not fire this event. |
| `error` | `{ error: Error }` | The adapter yielded an `error` chunk, or an internal error occurred during send. |
| `busy` | `{ busy: boolean }` | A response started (`true`) or settled (`false` — on `done` / `error` / `stop()`). Fires only on transitions. |
...
| `welcome-message` | string | — | Initial assistant greeting rendered when no stored history exists. Declarative sugar for `initialMessages` (stored history still wins). |
| `api-timeout` | number (ms) | — | Passed to the built-in adapter as `timeoutMs`. Ignored (treated as unset) unless a positive integer; also ignored when an `adapter` option is supplied. |
| `max-input-length` | number | — | `maxlength` applied to the input textarea. Ignored (treated as unset) unless a positive integer. Does not limit programmatic `sendMessage()`. |
...
| `api-timeout` | No — evaluated at mount time only |
| `welcome-message` / `max-input-length` | No — evaluated at mount time only |
...
| `stop-button` | The same button while a response is in flight — its `part` switches from `send-button` to `stop-button` |
| `badge` | Unread indicator on the FAB |
| `copy-button` | Copy button on assistant code blocks |
...
interface OpenAISseAdapterOptions {
  ...
  timeoutMs?: number;       // inactivity timeout — see below
...
- When `timeoutMs` is set, it bounds the wait for response headers **and** the inactivity gap between SSE reads (a steadily streaming response never times out). On timeout the adapter aborts its own fetch and yields an `error` chunk. Unset = no timeout (current behavior).
...
- When `timeoutMs` is set, it bounds the whole request (fetch + body read). On timeout the adapter aborts its own fetch and yields an `error` chunk. Unset = no timeout (current behavior).
...
### 6.1 `LabelDictionary` — all 19 keys
...
interface LabelDictionary {
  ...
  stopButton: string;     // stop-generation button label — en "Stop" / ja "停止"
  unreadBadge: string;    // sr-only text on the FAB unread badge — en "New message" / ja "新着メッセージ"
  copyCode: string;       // code block copy button label — en "Copy" / ja "コピー"
  copyCodeDone: string;   // copy button label after a successful copy — en "Copied" / ja "コピーしました"
}
```
